...
The v2.10.4 header's second line and the "(e-mail)" hint are removed: the prompt now reads "Enter your Acronis portal credentials (cloudbackup.datacomm.co.id)" followed by "Portal login:". Portal usernames are not always e-mail format, so the hint could mislead.
...
- Ships all tool outputs — CVT logs, acropsh logs/zip, Collect SysInfo
  `system_report_*` reports and the audit log — to another host via **SCP** as
  one timestamped tarball (`acronis-outputs_<host>_<ts>.tar.gz`).
- Acronis **installer `.bin` files are never included** (name-pattern gather).
- Interactive prompts: destination host / SSH user (default root) / port (22) /
  remote directory (default `~/acronis-collected`).
- `StrictHostKeyChecking=accept-new` — first connect doesn't hang on the
  yes/no prompt; spinner + elapsed while uploading.
- Tarball contents + size listed before sending; on completion the remote
  path and extract command are printed.

</details>

<details>
<summary><b>2.9.14</b> — FIX: spinner invisible during sysinfo collection</summary>

- 2.9.13 put `>/dev/null 2>&1` **outside** `run_bg` — that redirect silences
  the whole function, spinner included. Output redirect moved inside a
  `bash -c` wrapper: spinner prints to the terminal, the collector binary
  stays quiet. Verified by capturing raw spinner frames on stdout.

</details>

<details>
<summary><b>2.9.13</b> — Collect SysInfo: spinner + live elapsed counter</summary>

- The collection step ran fully silent for potentially minutes — on log-heavy
  machines indistinguishable from a hang. Both paths (systeminfo binary and
  acrocmd fallback) now show the standard tool spinner with a live
  `collecting system report ... Ns` elapsed counter (same `run_bg` used by
  downloads).
...
Root cause of the last ragged edge: the shield emoji inside the header box. Emoji column-width is **font/terminal dependent** — no padding math can be right everywhere. Fix:

- Header box interior is now **pure ASCII** (emoji removed) → box borders align on every terminal/font
- Column width computed locale-independently (ASCII count + non-ASCII runs), not `wc -m` (which counts bytes in C locale, codepoints in UTF-8 — never columns)
- Shield emoji 🛡️ moved to the one-shot intro splash, above the banner
